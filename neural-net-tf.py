...
from tensorflow.python.ops.summary_op_util import collect
...
def model_fn(features, labels, mode):
    ...
    eval_metric_ops = {
        "accuracy": tf.metrics.accuracy(\
            labels=labels, predictions=predictions["Label"])
    }
    return tf.estimator.EstimatorSpec(\
...
def main():
    ...
    train_input_fn = tf.compat.v1.estimator.inputs.pandas_input_fn(
        x=list(train_data),
        #x = pd.DataFrame(train_data, columns=["x"]),
        y=train_labels,
        batch_size=50,
        num_epochs=None,
        shuffle=True)
    ...
    predict_labels = [label["Label"] for label in predict_results]
