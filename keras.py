...
def baseline_model():
    ...
    model = Sequential()
    model.add(Dense(256, input_dim=100, activation='relu', kernel_initializer='normal'))
...
model.summary()
x_train, x_test, y_train, y_test = train_test_split(xtr, ytr, test_size=0.2,  stratify=ytr)
...
estimator = model.fit(x_train, encoded_y, epochs=5, batch_size=64)
print("Model Trained!")
